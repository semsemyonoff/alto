...
Mobile/tablet layout (`web/frontend/src/styles/responsive.css`, imported last in `index.css`) is driven entirely by classes on `<body>`, toggled from `web/frontend/src/mobile.ts` via document-level event delegation. Because htmx swaps only `#content-area` and never replaces `<body>`, these classes survive navigation. The four classes are `tree-open` (sidebar drawer), `dock-open` (transcode dock overlay), `has-sel` (a directory with tracks is shown ⇒ show the Transcode FAB), and `queue-open` (queue expanded ⇒ hide the FAB so it can't overlap). `queue-open` bridges Alpine's local `collapsed` state via `x-effect` on the `.queue` section in `base.html`. On every `htmx:afterSwap`/`historyRestore`, `syncSelection` recomputes `has-sel` from the presence of `#tc-dock` in the content area and *unconditionally* clears `dock-open` (a navigation swaps out the dock DOM, so a stale overlay must not linger). The tree-drawer auto-dismiss lives separately in `main.ts`: the `a.tree-label-link` click handler clears `tree-open` so opening a directory closes the drawer. Breakpoints: `≤1023` (shared tablet+mobile), `640–1023` (tablet drawer), `≤639` (mobile bottom-sheet). All rules live behind media queries; the desktop layout is untouched.
